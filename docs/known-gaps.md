...
- **Scout conclusions never make it back in the single-turn form (found by the long runs, not yet fixed).** A `SpawnScout` child runs to completion (`turn/end{completed}`), yet the parent session gets no `scout/settled` and the material surface gets no `source:'scout'` observation. Root cause: `AwaitWorldlines`' wait loop only counts **worldline executors** as "still running", while `sweepScouts()` never reports such a line — so in a scouts-only run the loop exits on its first tick, even though SpawnScout's own reply tells the model to "wait for it this turn with `AwaitWorldlines`". The one-shot form adds a second layer: with no next turn, a conclusion that settles after the last tool call never meets another collection point. The consequence is not a silent false green — the independent evaluator refuses any criterion that leans on the missing conclusion (observed as `inconclusive`) — but a product capability is genuinely missing. Details and the proposed fix: [long-run results and findings](optimization/e2e-longruns.md).
