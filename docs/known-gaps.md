...
- **Scout conclusions are invisible to the model (found by the long runs, not yet fixed).** The conclusion text lands only in the mutation record (visible to the ledger and the panel), while the section the model reads every step is the runtime card from `renderCard()` — and that card has **no material surface**: `state.materials` only ever reaches the panel view. Yet both `SpawnScout` and the kernel's own wording promise "you'll see it in the card's material surface next step". Consequence: reconnaissance is nearly useless to the model (it learns that a conclusion arrived, never what it says), and any criterion phrased as "consistent with the scout's conclusion" is unreadable for both the model and the independent evaluator, which can only return `inconclusive`. Details and reproduction: [long-run results and findings](optimization/e2e-longruns.md), finding 6.
