...
复跑时发现「执行者降级了」是**误读**:U2b 的改动脚本 `assert` 失败而我没检查输出就宣布成功,
提交信息里有、代码里没有——执行者从未尝试可续跑。补上真改动之外,顺手把**降级原因**落账
(`degraded_reason` + 诊断日志):读账的人必须能分清「部署限制」与「代码没落地」。

同时发现并修掉:真实部署里「去子会话日志捞结论」并不可靠(通知到了模型、账本却空),
改成**用原生结算通知当账本的结算信号**(它本来就在父会话日志里);执行者改走可续跑后暴露的
重发风暴;fold 通知分支里未声明的 `next`。

判据也跟着改准了两处:不可判是**设计内的诚实结局**(不该判成失败),
而恒真的断言(安全性质/条件断言)必须标出来——否则它们会冒充证据。

### 第三轮:统一子 run 生命周期(侦察/执行者/评估者/仲裁)

一个原生启动接口(`subagents.start()` 的一次性句柄)、一套生命周期、一条结果收集通道;
角色差异只剩人格/工具面/结果解释方式,权限与裁决边界不动。撤回上一轮的可续跑实验。

- 内核 605 项(含新增**会话隔离**用例:两个会话同时派任务不串账)
- 送达模型 = 收集那一刻的返回值带正文;子会话日志回收降为重启慢路径
- 判据修两处误报(套娃内容块、空白归一)并配真回归;剧本补上 CloseGoal 要求
- **端到端 3/3 全绿**(deepseek-flash 并行):scout-first 38/0 · worldline 40/0 · falsification 38/0

### 第三轮收尾:归档机制、事故与文档对齐

**归档机制(证据保全)**：长测改为**按场归档、永不覆盖**——轻档入库(run.log / summary.json /
meta.json / trajectory.txt + 追加式 index.json),重档留盘(`~/.dsh/e2e-archive/<剧本>/<时间戳>/`
下的工作区现场与会话日志原件,可用 `e2e-replay` 离线复算)。

**一次真事故(由归档改动引入,已修)**：我把 e2e 工作区放进了仓库内,而内核按设计
「工作区是 git 仓库时把交付提交进那个仓库」——于是它用 `clearai@local` 往本项目仓库提交了自己的
交付快照,并把当时未提交的两个文件一起扫了进去。处理:`git reset --mixed` 撤掉该提交(改动无损),
并把重型现场移出仓库;`e2e-run.mjs` 现在**当场拒绝**落在 git 仓库内的工作区(fail closed)。

**顺带被归档暴露的 bug**：会话目录名 slug **保留点号**(`~/.dsh/…` 里的 `.dsh`),
旧规则把它折成 `-`,导致一场跑动被误判成「没有会话日志」(38 项断言全红)。规则已改为
保留 `[A-Za-z0-9_.]`——两条(下划线、点号)都是**拿真实目录比对出来的**。

**文档对齐**：真值表 44 条行号清成符号(行号会漂),新增 `subrun-lifecycle` 一条机制
