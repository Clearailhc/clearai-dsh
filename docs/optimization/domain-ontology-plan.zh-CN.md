...
4. 图可以**渲染**（Markdown + Mermaid 货架、右栏本体页签的词汇图/知识图）并且**可编辑**——编辑一律落成**具名治理动词**，图编辑不写文件、不产生布局事件。
...
- **图是投影，不是存储**：本体与知识图都由 `fold` 算出；布局是确定性纯函数，不进账本。
...
- 能用中文案例完整讲清「概念 / 谓词 / 断言 / 事实 / 本体图 / 知识图」的区别；
...
- 右栏第四个页签「本体」（`sidebarRightTabs.register`）：词汇图 / 知识图 / 条目详情；
- 零依赖 SVG 渲染：缩放、平移、选择、视图切换、按概念/谓词/等级/状态筛选、废止幽灵、冲突高亮、实例确定性截断并标注数量；
- 事实页签断言芯片 → 本体页签互跳；概念节点「使用事实」 → 事实页签过滤；
- `clear/ontology/domain.md` 的 Mermaid 双图。

**验收**

- 同一账本在 Markdown 货架、运行卡、本体页签显示同一组节点/边/状态；
- 空本体、只有概念无谓词、只有谓词无事实、实例过多四类空/满状态都有明确文案；
- 双语 locale 键齐（`test/client.test.mjs` 覆盖）。

**回滚**：revert 页签注册与客户端改动；货架保留（只读，无副作用）。
...
- 设计文档与计划的双语同步；术语表增「领域本体 / 概念 / 谓词 / 断言 / 值形态 / 实例 / 知识图 / 冲突」；
...
真浏览器走查：注册概念与谓词 → 图上编辑 → 提出类型化假设 → 升格 → 知识图 → 冲突 → 撤回 → 刷新。
