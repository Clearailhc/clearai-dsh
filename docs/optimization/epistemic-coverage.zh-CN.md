...
| ② | 假设 ≥2，各带推翻条件 | `RegisterHypothesis` + prompts | **建议**（`minHypotheses: 0`） | **部分** | kernel 套件（只钉单条形状） |
...
| 拍 | 预期行为 | 状态 | 归宿 |
|---|---|---|---|
| ② | 假设数量下限（≥2）成为机制 | 部分 | 阶段 4b 落为硬边界，或明示降级为偏好 |
...
| — | 授权语义统一（见 §4） | 已实现但**文案自相矛盾** | 阶段 4 归一 |
...
当前系统在同一回合对模型说两句矛盾的话（`CreatePlan` 结果说「不要开工」，
运行态卡说「不需要任何人先按什么」）。归一决定，**行为不变，文案与推理统一**：
...
- **落为机制**：假设数量下限（阶段 4b）；权威边界测试（阶段 4）；`/` 菜单与非权威能力
  （阶段 5）；`autonomy.override` 死路径（阶段 4/5 决定删或留，留则写明只读旧日志）。
...
```text
人: 立目标
系统: 澄清(槽位措辞) → SetGoal → 登记假设 → CreatePlan(判据校验) → 原生审阅卡
人: 批准 / 改意见 / 撤下
系统: 布防续跑令牌(goals) → 执行步骤 → 产物落盘
系统: 观测准入 → needs_audit? → 派独立评估者(只读工具面+outputSchema)
评估者: 评估卡落盘(支持/推翻/无法判定)
系统: 驳回? → 计数+1 → ≥阈值则 blocked 等人
系统: 证据齐 → 事实升格 → 结案评估 → 收兵/补防
系统: turnDemand: 门开→hold；门关→宿主开下一轮(≤128)
