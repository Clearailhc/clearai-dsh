...
| 图是投影，不是存储 | `ui/lib/domain-language.js`（`graphProjection` / `deriveConflicts` / `lexiconHealth`）；`ui/lib/fold.js` 的 `state.lexicon` 与 `view().lexicon`；布局是确定性纯函数 | `test/domain-language.test.mjs`（同账本同图、冲突只暴露、旧账本兼容）；折法层已实现；生产动词、货架与面板是[开发计划](optimization/domain-ontology-plan.zh-CN.md)阶段 C–E 的设计目标。 |
