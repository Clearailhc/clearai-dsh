...
2. **The evidence gate rests on criteria, not levels, so it can be bypassed by "no criteria".** Dispatching an independent evaluator is triggered by `needs_audit` (artifacts complete **and** `done_criteria` non-empty). A step at L3 or above without `done_criteria` takes the deterministic release exit and **never passes through independent evaluation**. That is the soft spot in "write the criteria before doing the work": it assumes the criteria were written.
