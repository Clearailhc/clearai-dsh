...
| 面板「本体」 | 词汇图 / 知识图 / 条目详情 | 投影 `view().lexicon`（阶段 D–E 渲染） |
...
1. **词汇图**：概念节点、`is_a` 层级、谓词边、值形态；
2. **知识图**：实例、事实边、等级、证据、冲突高亮；
3. **条目详情**：选中节点或边后的释义、版本、依据、使用事实、修订/废止入口。

**本体页签只治理领域本体。** 过程本体不作为可编辑内容出现在这里——它的呈现是世界树里的步骤与闸门，以及命题按状态分组；模型读的章程是 `clear/ontology/verification-loop.md` 货架。

第一版使用客户端零依赖 SVG：缩放、平移、选择、视图切换、按概念/谓词/等级/状态筛选、冲突高亮、废止幽灵显示、实例过多时确定性截断并明确标注截断数量。

**编辑采用「详情抽屉 + 应用变更」**，不做拖拽连边：连边手势很容易表达出错误的语义，而抽屉可以明确要求域、值域、值形态与依据。拖拽连边是后续增强，不是第一版的可用性前提。

互跳：事实行的断言芯片 → 本体页签并选中谓词；概念节点的「使用事实」→ 事实页签按概念过滤；知识图事实边 → 事实详情、证据与世界树验证步骤；冲突边 → 成对打开两条事实。
