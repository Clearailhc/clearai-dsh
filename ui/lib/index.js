 ...
 * 东西持有进程级资源——第二次挂载必然撞名(实测:webserver duplicate exact route)。
 *
...
/** 浏览器读到的那个视图(面板契约,见 DESIGN.md §3.2)。 */
...
export function apply(ctx) {
	 ...
	 *      `set_autonomy` 于 §34 摘掉:「在场与否」是运行时状态,不该是面板上的一个开关 ✗;
	 ...
	 *
	 * 2026-09-11 砍掉两个动词(奥卡姆:它们是重复,不是能力):
	 ...
	 * 没有它(实测:全仓 `host.call` 只出现在动态运行时的两个包里)。宿主平面的路由是这条
	 ...
	 *
	 * 为什么要它(2026-09-11 用户实测):在一个已经有产物的老项目里开新会话,「产物」栏只有
	...
	ctx.inject(['connection'], (connectionCtx) => {
		...
		route('/api/clearai/gate', ['POST'], async (httpRequest) => {
			...
			/**
			 * `ask` 是**界面手势**,不是事实动词(§20):它把某道等人的人门摆到**原生提问卡**上,
			...
			const followUp =
				action === 'promote_skill'
					...
					/* 说准(rather than 说满):这句话写下的那一刻只到 inbox、还没进投影,
					   所以不能断言「已经在投影里生效」——实测里模型看到它与卡片矛盾,停下来问过人(§17.5)。 */
		 ...
		 *
		 * **没有「读文件正文」那条了**(2026-09-11 删):预览交给 DSH 原生的文档预览
			...
			const stages = (state?.plans ?? []).map((plan) => ({
				...
				steps: (plan.steps ?? []).map((step) => ({
					...
					artifacts: (step.artifacts ?? []).map((artifact) => {
						...
						/**
						 * `exists: false` 有两种成因,**面板必须分开说**(2026-09-11 长测抓到的):
		 ...
		 *
		 * 为什么需要它(2026-09-11 用户实测):面板的数据来自会话日志的投影,而内核只在
	...
	ctx.effect(
		() =>
			ctx.provide('clearai', {
				 ...
				 * 运行态卡:注给模型的**事实**。
				 *
				 * `overrides.autonomy`:人刚在面板上切过档的那一拍,投影里还是旧档(消息先入 inbox、
				 * 后落日志,而 pre-step 跑在它落账之前,§17.5)。卡片必须与这一回合**真正要跑的机制**
				 * 同源,否则模型会读到两条互相矛盾的话——实测里它为此停下来问过人。
				 */
