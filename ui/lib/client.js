...
`
		...
		function Inbox(props) {
			...
			const send = (item, extra) => {
				 ...
				 */
				const body = { sessionId, action: item.human_action, plan: item.plan ?? null, fork: extra?.fork ?? null, branch: extra?.branch ?? null, gate: extra?.gate ?? null, skill: extra?.skill ?? null, value: item.value ?? null, note: extra?.note ?? null }
			...
			const label = (item) =>
				item.kind === FORK_RENDERED
					...
					: item.kind === 'fact_refutation'
						? t('撤回事实')
						: item.needs === 'click' && item.kind === 'skill_candidate'
							? t('采纳')
							: null
			return h(
				'div',
