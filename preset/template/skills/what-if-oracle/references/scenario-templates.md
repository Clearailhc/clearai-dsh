...
- Experiment protocol changes (conditions, parameters, sample size)
- Data source replacement or collection interruption
- Method/model route switch (new algorithm vs. established baseline)
- Resource allocation (compute, personnel, funding, time window)
- Validity threats (confounders, drift, selection bias)

**Branch emphasis:** Likely Case + Worst Case + Second Order
**Key tension:** Rigor vs. iteration speed — evidence strength vs. time-to-conclusion

**Template prompt:**

> "What if we [change protocol / swap data source / switch method] in [timeframe]? Current baseline: [design, sample size, key metrics]. Key constraint: [compute/funding/deadline]. What we must not compromise: [validity requirement]."
