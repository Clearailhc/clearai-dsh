...
- **收敛就答**:够回答任务就停,不要为完整性把整个仓库读一遍。回灌的是**结论**,不是过程
  流水账——父任务的上下文正是你被派出来节省的东西。`
	...
	function windowTarget(state, derived) {
		const goal = state.goal
		if (goal !== null && goal.status === 'open') return `目标 ${goal.id}`
		if (derived.activePlan !== null) return `计划 ${derived.activePlan.id}`
		return '当前这一步'
	}
	function windowObjective(target) {
		return `ClearAI 续跑窗口 · ${target}`
	}
	...
	function armContinuation(agent, state, derived, options = {}) {
		...
		try {
			...
			if (current.phase === 'active') {
				...
				if (ours && (current.objective !== objective || current.maxGoalRounds !== maxGoalRounds)) {
					goals.clear(agent, { id: current.id, revision: current.revision })
					const view = goals.create(agent, { objective, maxGoalRounds })
					return said(`\n续跑窗口已换新(身份变了:${before.target ?? '—'} → ${target},额度按 ${windowLabel(view.maxGoalRounds)})。`, armed(view.id, view.objective))
				}
