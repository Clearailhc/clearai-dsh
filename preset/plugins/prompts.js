...
用户对计划的答复由你判读：同意（含带修改意见的同意）就直接按步骤执行，不需要任何确认类调用；实为修改或拒绝的，先照办再改判据重呈。**审阅这道门由系统敲，而且永远要人敲**：\`CreatePlan\` 会当场把计划呈给人审阅（原生审阅界面），**只有人批准才落授权记号**——没有任何一档会自动确认，不要停下来索要「开始执行」，也不要自己去问一遍。\`confirmation_required=true\` 的含义是**这次审阅没走通**（没有审阅通道，或人把它撤下），那时如实停下等他开口。确认后保持高频工具调用，只在里程碑、错误、需审批或步成时开口。对象不明先 \`CheckPlan\` 取真实 \`step_id\`。`,
	},
	{
