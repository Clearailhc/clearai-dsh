...
console.log('\n【无人值守续跑:宿主目标只当驱动器,不当事实源】')
{
	 ...
	 */
	check(
		'续跑窗口的文本是身份(服务对象),不冒充用户的目标,也不写档位',
		/^ClearAI 续跑窗口 · 目标 g-[a-z0-9]+$/.test(String(unattended.hostGoal?.objective ?? '')) && !/催化剂 A/.test(String(unattended.hostGoal?.objective ?? '')),
	...
	check(
		'布防落账:投影里有 continuation=armed 与它服务的对象',
		unattended.service.state(U).continuation?.state === 'armed' && /^目标 g-[a-z0-9]+$/.test(String(unattended.service.state(U).continuation?.target ?? '')),
		JSON.stringify(unattended.service.state(U).continuation),
	)

	// 修订目标:窗口**一个字都不动**(不 edit、不新建)——身份没变就不该换窗口,
	// 而换窗口会重置轮数,那等于「反复修订目标就能刷预算」。
	...
	check('修订目标 → 不动窗口(不 edit、不 clear、不 create)', g2.ok === true && calls().slice(before).every((name) => name === 'get'), calls().slice(before).join(','))
	...
	check(
		'人开口 ⇒ 换新窗口,新窗口说新身份(账上如实写「已换新」)',
		goalNames(rewritten, beforeTurnover).includes('clear') && goalNames(rewritten, beforeTurnover).includes('create') && /^ClearAI 续跑窗口 · 目标 g-[a-z0-9]+$/.test(String(rewritten.hostGoal?.objective)),
