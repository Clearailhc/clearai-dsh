...
for round in $(seq 1 "$ROUNDS"); do
	...
	printf '第 %s 遍 · 内核:%s · 宿主:%s · 外脑:%s · 客户端:%s · 本体:%s · 真值表:%s · 状态机:%s · 文档:%s · 注释:%s · 边界:%s · 组合:%s · 段:%s\n' "$round" "${kernel_line:-崩溃}" "${host_display:-崩溃}" "${brain_line:-崩溃}" "${client_display:-崩溃}" "${ontology_line:-崩溃}" "${truth_line:-崩溃}" "${statemachine_line:-崩溃}" "${docs_line:-崩溃}" "${comment_line:-崩溃}" "${authority_line:-崩溃}" "${composition_line:-崩溃}" "${sections_line:-崩溃}"
	if ! printf '%s\n' "$kernel_line" | grep -qa ',0 失败' || ! printf '%s\n' "$brain_line" | grep -qa ',0 失败' || ! printf '%s\n' "$ontology_line" | grep -qa ',0 失败' || ! printf '%s\n' "$truth_line" | grep -qa ',0 失败' || ! printf '%s\n' "$statemachine_line" | grep -qa ',0 失败' || ! printf '%s\n' "$docs_line" | grep -qa ',0 失败' || ! printf '%s\n' "$comment_line" | grep -qa ',0 失败' || ! printf '%s\n' "$authority_line" | grep -qa ',0 失败' || ! printf '%s\n' "$composition_line" | grep -qa ',0 失败' || ! printf '%s\n' "$sections_line" | grep -qa ',0 失败'; then
		FAILED=1
		printf '\n--- 内核 ---\n%s\n--- 外脑 ---\n%s\n--- 本体 ---\n%s\n--- 真值表 ---\n%s\n--- 状态机 ---\n%s\n--- 文档 ---\n%s\n--- 注释 ---\n%s\n--- 边界 ---\n%s\n--- 组合 ---\n%s\n--- 段 ---\n%s\n' "$kernel" "$brain" "$ontology" "$truth" "$statemachine" "$docs" "$comment" "$authority" "$composition" "$sections"
	fi
