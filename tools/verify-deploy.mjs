 ...
 * 组合文件 mtime 变化时重建。于是 `clearai_preset_check` 会拿**缓存里的旧模块**去挂,报出来的错
 * (第 9 轮实测:`webserver: duplicate exact route "/api/clearai/loop"`——那是投影化之前的代码)
 ...
 * 当前实测:20 件工具(计划/目标九件 + 世界线五件 + 侦察两件 + 外脑两件 + 账本两件)、22 段(段表 23,澄清协议两套互斥取一)。
 * 它**不**证明运行期行为:那需要重启宿主(见 ROADMAP 开发纪律 1)。
 *
 * 第 24 轮起,它**也验宿主包**:用部署的那一份 + 假 ctx 装配一次,数「面板的三条路由是否挂在
 * connection 的 exact fetch 表上」(挂错层 = 浏览器里一按就 404 "not found",实测踩过两次)。
 *
